...
- Item_MRP (price) is by far the strongest predictor of Item_Outlet_Sales (sales). It predicts sales
- The next best predictors of sales are all related to the outlet. Outlet_Type, Outlet_Identifier, and Years_Open all predict sales responibly well.  
...
ggplot(total_sales, aes(Outlet_Identifier, Total_Sales)) +
    ...
    theme(panel.grid.major.y = element_line(color = 'gray30'))
```
...
Finally, we ran a random forest model. It preforms far better than the baseline model and the lasso regression.
```{r}
