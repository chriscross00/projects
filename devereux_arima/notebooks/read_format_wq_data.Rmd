...
conversion_func <- function(df){
  index <- sapply(df[, 3:8], is.factor)
  df[index] <- lapply(df[index], function(x) as.numeric(levels(x))[x])
  return(df)
}
...
```{r}
what <- create_arima_ready(test_path)

head(what)
```

```{r}
just_factor <- function(df) {
  for (i in list(3:8)) {
    if (is.factor(df[, i]) == TRUE) {
      df[, i] <- as.numeric(levels(df[, i]))[df[, i]]
    }
