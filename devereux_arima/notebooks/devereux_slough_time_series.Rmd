...
some water moleculse to evaporate. 

Evaporation will peak during This process will If we graphed air temperature 
alongside water level we would expect to see that the two lines are highly
correlate, with the water level cycles closely following the air temperature
cycles.
...
notebook we can rerun the entire analysis with 3 months of continuous data. The
forecast we get are []
